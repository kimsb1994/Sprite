    ...
    void loadFromFile(int resource) {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.startsWith("VOLADOR=")) {
                    String[] parts = line.split("=");
                    String[] coors = parts[1].trim().split(",");
                    Volador volador = new Volador(game);
                    volador.x1 = Integer.parseInt(coors[0].trim()) * 16;
                    volador.x2 = Integer.parseInt(coors[1].trim()) * 16;
                    volador.y = Integer.parseInt(coors[2].trim()) * 16 - 6;
                    volador.x = volador.x1;
                    game.addVolador(volador);
                    continue;
                }
